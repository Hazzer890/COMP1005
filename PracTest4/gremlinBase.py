...
def main():
    ...
    water = [(9,0),(9,29),(19,15),(0,15),(9,15)]
    ...
    # Starting population
    if (len(sys.argv) < 3):
      #  print('argv too short, usage: python3 <Population> <Show Plot Y/N>')
       #print('Using default values for interval (8) and dosage (100)')
        maxpop = 15
        showPlot = 'Y'
    else:
        maxpop = int(sys.argv[1]) 
        showPlot = str(sys.argv[2]).upper()
    

    for i in range(maxpop):  # add good gremlins to grid
        goodarray[random.randint(0,MAXROW-1),random.randint(0,MAXCOL-1)] += 1 

    # Simulation
    
    for t in range(24):    # @ 8 hour / timestep = 8 days
        #print("### Timestep ", t, "###")
    ...
    print(maxpop, ',', goodarray.sum(), ',', evilarray.sum())
    figname = "Gremlins@" + str(maxpop) + ".png"
    plt.style.use = 'default'
    make_scatter(goodarray, "b")
    make_scatter(evilarray, "r")
    make_map(water, "c")
